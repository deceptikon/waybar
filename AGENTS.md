...
## Structure
- **Config files**: One JSON file per bar (`config-top`, `config-vertical`, `config-bottom`).
...
- **CSS**: `style/base.css` (shared layout), `style/top.css` (top bar overrides + inline Catppuccin @define-colors), `style/vertical.css`, `style/bottom.css`.
- **Scripts**: `scripts/waybar-start.sh` starts all 3 bars sequentially (0.5s gaps) — full restart on reload (pkill + start). Other scripts in `scripts/utils/`, `scripts/network/`, `scripts/sysmon/`.
...
- **Per-bar D-Bus names**: `bar-top` vs `bar-top-ext` avoids name conflicts within a single waybar process.
