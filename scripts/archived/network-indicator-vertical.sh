...
# For vertical layout, we output the progress bar on first line, text on second
# Using newline to separate them
echo "{\"text\":\"$progress_bar\\n$net_icon ↓${rx_formatted}K ↑${tx_formatted}K\",\"tooltip\":\"Network ($interface): ↓${rx_formatted}KB/s ↑${tx_formatted}KB/s\nLine on top, digits on bottom\",\"class\":\"$net_class\"}" | jq --compact-output