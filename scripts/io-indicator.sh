...
echo "{\"text\":\"$io_icon $progress_bar ↓${read_formatted} ↑${write_formatted}\",\"tooltip\":\"IOPS: $iops | Read: ${read_formatted} MB/s | Write: ${write_formatted} MB/s\",\"class\":\"$io_class\"}" | jq --compact-output